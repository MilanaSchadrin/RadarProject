...
class SkyEnv:
    ...
    def check_collision(self, rocket:Rocket):
        ...
        maxTime = min(len(planetarjectory), len(rockettrajectory)+int(rocket.get_startTime()))
        for t in range(int(maxTime)):
            if t<rocket.get_startTime():
                continue
            trIndex = t - int(rocket.get_startTime())
            positionPlane = planetarjectory[t]
            positionRocket = rockettrajectory[int(trIndex)]
