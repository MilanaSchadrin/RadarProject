...
class Rocket(SkyObject):
    ...
    def calculate_trajectory(self):
                ...
                # Инициализируем массив траектории правильного размера (timeSteps x 3)
                self.trajectory = np.zeros((self.timeSteps, 3))

                # Заполняем начальные значения
                self.trajectory[:, :] = np.array(self.start)  # Копируем стартовую точку во все строки

                # Рассчитываем движение по x и y (без сопротивления воздуха)
                self.trajectory[:, 0] += self.velocity[0] * timeReal  # X координата
                self.trajectory[:, 1] += self.velocity[1] * timeReal  # Y координата

                # Рассчитываем движение по z (с учетом гравитации)
                if self.gravity != 0:
                    self.trajectory[:, 2] += (self.velocity[2] * timeReal +
                                           0.5 * self.gravity * timeReal**2)
                else:
                    self.trajectory[:, 2] += self.velocity[2] * timeReal
    def get_radius(self):
        return self.radius
