...
class MapWindow(QMainWindow):
    def __init__(self, db_manager):
        ...
        self.rocket_animation_duration = 600
        ...
        self.pre_explosion_states = {}
        self.destroyed_planes = {}
    ...
    def visualize_zur_track(self, zur_id, coords: np.ndarray):
           self.text_output.append(f'<span style="color: green;">🚀 Ракета с ID {zur_id} появилася в воздушном пространстве</span>')
           if coords is None or len(coords) == 0:
                return
           coords = np.atleast_2d(coords)
           if coords.shape[1] < 2:
    ...
    def process_message(self, msg):
           try:
                if msg['type'] == 'plane_start':
                    ...
                    #print("RADAR", msg['data'])
                    radar_id = msg['data'].radar_id
                    target_id = msg['data'].target_id
                    if radar_id not in self.radar_targets:
                         self.radar_targets[radar_id] = set()
                    self.radar_targets[radar_id].add(target_id)
                    #print("SELF RADAR_TARGET", self.radar_targets)

                    self.map_view.handle_target_detection(radar_id, target_id, msg['data'].sector_size)
                    self.text_output.append(f'<span style="color: orange;"> ❗Обнаружена цель {target_id} 📡 радаром {radar_id}')
                    #self.update_radar_targets(self.radar_targets)
    ...
    def process_plane_destruction(self, plane_id: int, cross_label: QLabel, collision_step: int):
        ...
        self.text_output.append((f'<span style="color: red;">✈️ Самолет с ID {plane_id} был сбит ракетой 🎯'))
        pos = cross_label.pos()
        ...
        for radar_id in self.radar_targets:
                if plane_id in self.radar_targets[radar_id]:
                    self.radar_targets[radar_id].remove(plane_id)
                    del self.map_view.tracked_targets [plane_id]
