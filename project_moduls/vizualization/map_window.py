...
class MapView(QGraphicsView):
    ...
    def handle_target_destruction(self, explosion_data):
            """Полностью удаляет уничтоженные цели из всех структур"""
            # Удаляем из tracked_targets
            for radar_id, targets in list(self.tracked_targets.items()):
                if explosion_data.plane_id in targets:
                    ...
                    if not targets:  # Если больше нет целей для этого радара
                        del self.tracked_targets[radar_id]

            # Удаляем трейлы уничтоженных объектов
            if hasattr(self, 'trails'):
                for obj_id in [explosion_data.plane_id, explosion_data.rocket_id] + \
                             [dmg[0] for dmg in explosion_data.collateral_damage]:
                    if obj_id in self.trails:
                        # Удаляем графические элементы трейла
                        for point_item in self.trails[obj_id]:
                            if point_item in self.scene.items():
